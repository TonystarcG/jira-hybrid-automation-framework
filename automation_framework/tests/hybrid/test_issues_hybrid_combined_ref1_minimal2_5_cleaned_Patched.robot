...
Create Issues And Upload Attachment
    ...
    Log Many    BASE_URL=${BASE_URL}    EMAIL=${EMAIL}    API_TOKEN=${API_TOKEN}    PROJECT_KEY=${PROJECT_KEY}    ISSUES_FILE=${ISSUES_FILE}
    ...
    FOR    ${issue}    IN    @{issues}
        ...
        Log    Attachment Upload Response: ${upload_response.content}
        ...
        Should Be Equal As Integers    ${result.rc}    0    msg=Playwright script failed with return code ${result.rc}
    END
...
Workflow Status Transition Test
    ...
    Log    EMAIL: ${EMAIL}
    Log    API_TOKEN: ${API_TOKEN}
    Log Many    BASE_URL=${BASE_URL}    EMAIL=${EMAIL}    API_TOKEN=${API_TOKEN}    PROJECT_KEY=${PROJECT_KEY}
    ...
    Should Be Equal As Integers    ${result.rc}    0    msg=Status transition script failed
    Log    Transition Script STDERR: ${result.stderr}
    ...
    Should Be Equal As Integers    ${ui_result.rc}    0    msg=UI validation script failed
